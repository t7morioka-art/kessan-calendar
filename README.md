...
**テーブルごとに書き手を1つに固定する。** 同じデータが2箇所から書かれることはない。

| テーブル | 書き手 | 内容 |
|---|---|---|
| `stocks` | 画面のみ | 銘柄マスタ（Actionsは読むだけ） |
| `earnings` | Actionsのみ | J-Quantsが取得した決算日 |
| `manual_earnings` | 画面のみ | 手入力の決算日 |
| `sync_runs` | Actionsのみ | 実行ログ |
| `listed` | 初期投入のみ | 証券コード→社名の対応表 |

手入力の決算日を `earnings` と分けているのは、Actionsの更新で消えないようにするため。
表示のときだけ UNION で統合する。
...
| URL | 内容 |
|---|---|
| `/` | 一覧（次の決算・今後の予定・日付待ち・対象外） |
| `/calendar` | 月カレンダー（`?ym=YYYY-MM` で月を指定） |
| `/stocks` | 銘柄の追加・削除、決算日の手入力 |

銘柄を追加するときは証券コード4桁を入れるだけでよい。社名は `listed` から自動で入り、
上場していないコードは弾かれる。
